...
public class ProductRestAdmin {
    ...
    @Operation(summary = "Finds a product by label")
    @GetMapping("label/{label}")
    public ResponseEntity<ProductDto> findByLabel(@PathVariable String label) {
	    Product t = service.findByReferenceEntity(new Product(label));
        if (t != null) {
            ProductDto dto = converter.toDto(t);
            return getDtoResponseEntity(dto);
        }
